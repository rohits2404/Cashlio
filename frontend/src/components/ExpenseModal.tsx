...
export const ExpenseModal = ({
    ...
    onAddExpense: (ExpenseData: ITransactionData) => void;
    onUpdateExpense: (ExpenseData: ITransactionData) => void;
    selectedExpense: ITransactionData | null;
    open: boolean;
    ...
    useEffect(() => {
        if (open) {
            if (selectedExpense) {
                setSelectedEmoji(selectedExpense.emoji);
                setTitle(selectedExpense.title);
                setCategory(selectedExpense.category);
                setAmount(String(selectedExpense.amount));
                setDate(selectedExpense.date ? new Date(selectedExpense.date) : null);
            } else {
                resetForm();
            }
        }
            ...
            <DialogContent className="sm:max-w-md">
                ...
                <div className="flex flex-col gap-5 mt-2">
                    ...
                    <div className="w-full">
                        ...
                        <Input className="mt-2" placeholder="Enter Expense Title" value={title} onChange={(e) => setTitle(e.target.value)} />
                    </div>
                    <div className="w-full">
                        ...
                        <Select onValueChange={(value) => setCategory(value)} value={category}>
                            ...
                            <SelectContent>
                                <SelectGroup>
                                    ...
                                    {EXPENSE_CATEGORY_CONSTANTS.map((item) => (
